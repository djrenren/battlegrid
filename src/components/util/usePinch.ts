...
function calcState(touches: TouchList, touch1: number, touch2: number) {
  ...
  for (let t of touches) {
    if (t.identifier === touch1 || t.identifier === touch2) {
      ...
      const center = add(found as any, found2 as any).map((n) => n / 2) as [
        number,
        number
      ];
      return { dist, center };
  ...
  useEffect(() => {
    const onTouchStart = (ev: TouchEvent) => {
      ...
      if (
        ...
        const touch =
          ev.touches[0].identifier === touch1.current
            ? ev.touches[1]
            : ev.touches[0];
        touch2.current = touch.identifier;

        const { center, dist } = calcState(
          ev.touches,
          touch1.current,
          touch2.current
        )!;
    ...
    const onTouchMove = (ev: TouchEvent) => {
      if (touch1.current === null || touch2.current === null) {
        return;
      }
    ...
    const onTouchEnd = (ev: TouchEvent) => {
      ...
      for (let t of ev.touches)
        if (
          t.identifier === touch1.current ||
          t.identifier === touch2.current
        ) {
          count += 1;
        }
      if (count !== 2) {
